...
func jsonPathLookup(ctx *Context, obj interface{}, jpath string) (interface{}, error) {
	c, err := compile(jpath)
	if err != nil {
		return nil, err
	}
...
func (c *compiled) Lookup(ctx *Context, obj interface{}) (interface{}, error) {
	var err error
	for _, s := range c.steps {
		...
		case "idx":
			...
			if len(s.args.([]int)) > 1 {
				...
				//fmt.Println("idx ----------------3")
				obj, err = get_idx(ctx, obj, s.args.([]int)[0])
				if err != nil {
					return nil, err
				}
			} else {
		...
		case "range":
			...
			if argsv, ok := s.args.([2]interface{}); ok == true {
				obj, err = get_range(ctx, obj, argsv[0], argsv[1])
				if err != nil {
					return nil, err
				}
			} else {
...
func filter_get_from_explicit_path(ctx *Context, obj interface{}, path string) (interface{}, error) {
	...
	for _, s := range steps {
		...
		case "idx":
			...
			xobj, err = get_idx(ctx, obj, args.([]int)[0])
			if err != nil {
				return nil, err
			}
		default:
...
func zgg_get_key(ctx *Context, val Value, key string) (interface{}, error) {
	if arr, isArray := val.(ValueArray); isArray {
		res := NewArray(arr.Len())
		for i := 0; i < arr.Len(); i++ {
			if item, err := zgg_get_key(ctx, arr.GetIndex(i, ctx), key); err == nil {
				res.PushBack(item.(Value))
			}
		}
		return res, nil
	} else {
		v := val.GetMember(key, ctx)
		if IsUndefined(v) {
...
func get_key(ctx *Context, obj interface{}, key string) (interface{}, error) {
	if val, is := obj.(Value); is {
		return zgg_get_key(ctx, val, key)
	}
	...
	case reflect.Slice:
		...
		for i := 0; i < reflect.ValueOf(obj).Len(); i++ {
			tmp, _ := get_idx(ctx, obj, i)
			if v, err := get_key(ctx, tmp, key); err == nil {
				res = append(res, v)
			}
		}
		return res, nil
...
func get_idx(ctx *Context, obj interface{}, idx int) (interface{}, error) {
	if val, is := obj.(Value); is {
		item := val.GetIndex(idx, ctx)
		if IsUndefined(item) {
			return nil, fmt.Errorf("index value undefined")
		}
		return item, nil
	}
	switch reflect.TypeOf(obj).Kind() {
...
func _get_range_params(length int, frm, to any) (_frm int, _to int) {
	_frm = 0
	_to = length
	if frm == nil {
		frm = 0
...
func get_range(ctx *Context, obj, frm, to interface{}) (interface{}, error) {
	if val, is := obj.(Value); is {
		if vc, canLen := val.(CanLen); canLen {
			length := vc.Len()
			_frm, _to := _get_range_params(length, frm, to)
			res := NewArray(length)
			for i := _frm; i < _to; i++ {
				item := val.GetIndex(i, ctx)
				if IsUndefined(item) {
					return nil, fmt.Errorf("index value undefined")
				}
				res.PushBack(item)
		...
		return nil, fmt.Errorf("val cannot Len")
	}
	switch reflect.TypeOf(obj).Kind() {
	case reflect.Slice:
		length := reflect.ValueOf(obj).Len()
	...
	case reflect.Slice:
		if op == "=~" {
			...
			for i := 0; i < reflect.ValueOf(obj).Len(); i++ {
				...
				ok, err := eval_reg_filter(ctx, tmp, root, lp, pat)
				if err != nil {
					return nil, err
				}
				if ok == true {
			...
			for i := 0; i < reflect.ValueOf(obj).Len(); i++ {
				tmp := reflect.ValueOf(obj).Index(i).Interface()
				ok, err := eval_filter(ctx, tmp, root, lp, op, rp)
				if err != nil {
					return nil, err
				}
				if ok == true {
...
func eval_reg_filter(ctx *Context, obj, root interface{}, lp string, pat *regexp.Regexp) (res bool, err error) {
	if pat == nil {
...
func get_lp_v(ctx *Context, obj, root interface{}, lp string) (interface{}, error) {
	var lp_v interface{}
...
func eval_filter(ctx *Context, obj, root interface{}, lp, op, rp string) (res bool, err error) {
	...
	if op == "exists" {
		...
		if strings.HasPrefix(rp, "@.") {
			rp_v, err = filter_get_from_explicit_path(ctx, obj, rp)
		} else if strings.HasPrefix(rp, "$.") {
			rp_v, err = filter_get_from_explicit_path(ctx, root, rp)
		} else {
			rp_v = rp
		}
